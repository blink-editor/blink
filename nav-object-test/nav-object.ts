...
var navObject = {
    ...
    rebuildMaps: function() {
        ...
        for (let docSym of result) {
            let symKey: string = this.encodeKey(docSym.name, docSym.kind, docSym.location.uri);
    ...
    findCallers: function(symPos: any) { // pass position not any
        var output = [];
    ...
    findCallees: function() { // idk what we need to pass yet but we will need something
        ...
        var acceptableSyms: number[] = [1, 2]; // add whatever we want
        var result = [
            { label: "class1", kind: 1 },
            { label: "func1", kind: 2 },
            { label: "var1", kind: 3 },
        ...
        for (let completion of result) {
            if (acceptableSyms.indexOf(completion.kind) >= 0) {
                ...
                for (let key in this.symToDefRange) {
                    let decodedKey: string[] = this.decodeKey(key);
